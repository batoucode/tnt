...
function displayVersion() {
    ...
    if (versionDisplay) {
        ...
        const version = "2026.02.08.07.30";
    ...
    fetch(jsonPath + '?t=' + timestamp, { cache: 'no-cache' })
        ...
        .then(textData => {
            ...
            try {
                ...
                // {
                //   "equipe": "U13M1",
                //   "titre du dernier_match": "TNT 45 - 47 Membrollais",
                //   "resultat TNT": "45",
                //   "resultat equipe adverse": "47",
                //   "dates et heure du prochain match": "7 mars 2026 à 01h00",
                //   "lieux de la prochaine rencontre": "à domicile",
                //   "commentaire": "Défaite serrée à l'extérieur",
                //   "maj": "8 février 2026 à 00h48"
                // }

                const team1Name = "TNT U13 M1";
                let team2Name = "Adversaire";
                let score1 = 0;
                let score2 = 0;

                // Extraire le nom de l'équipe adverse depuis "titre du dernier_match"
                if (data["titre du dernier_match"]) {
                    const titre = data["titre du dernier_match"];
                    // On cherche ce qui est après le score ou après "TNT XX - XX"
                    const parts = titre.split(/[\d]+\s*[-]\s*[\d]+/);
                    if (parts.length > 1) {
                        team2Name = parts[1].trim();
                    }
                }

                score1 = parseInt(data["resultat TNT"]) || 0;
                score2 = parseInt(data["resultat equipe adverse"]) || 0;

                const nextMatchInfo = `${data["dates et heure du prochain match"] || ""} ${data["lieux de la prochaine rencontre"] || ""}`.trim();

                // Créer l'objet score U13M1
                const u13m1Score = {
                    id: 1,
                    team1: team1Name,
                    team2: team2Name.toUpperCase(),
                    score1: score1,
                    score2: score2,
                    date: data.maj || new Date().toLocaleDateString('fr-FR'),
                    competition: "Championnat Départemental",
                    nextMatch: nextMatchInfo || "À venir",
                ...
                if (u13m1Team) {
                    u13m1Team.nextMatch = nextMatchInfo;
                    renderTeams();
                }
            } catch (e) {
                console.error("❌ Erreur de parsing JSON:", e);
                console.error("Contenu reçu non-JSON:", textData);
            }
        })
        .catch(error => {
